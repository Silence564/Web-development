...
export class NewsService {
  ...
  add_news(id:number, news_text: string, news_photo: string, on_add:() => void, on_error: (msg:string)=>void){
    this.http.post('http://localhost:4300/add_news', { id, news_text, news_photo}).subscribe((res:any) => { if(!res.error || res.error.length == 0) on_add(); else (on_error(res.error))})
  }
  onGetNews(socketfunc:()=> void){
    socket.removeAllListeners('news')
    socket.on('news', (message)=> socketfunc()) //клиент ловит событие
  }
}
